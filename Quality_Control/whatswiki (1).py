...
############################################################
# NETS 213: WhatsWiki Quality Control
############################################################
...
def majority_vote(wiki_res):
    ...
    for index, row in wiki_res.iterrows(): # put every attr-adj pairing in dictionary with labels
      for i in range(1, 11):
        ...
        description = row['Answer.description_' + str(i)]

        
        descriptions = []
        if title not in labels: # put in dictionary
          labels[title] = [0, descriptions];
        ...
        if answer == "Yes":
          ...
          labels[title][0] -= 1
        # ignore blanks
        labels[title][1].append(description);

    result = []
    # make the result list of tuples
    for title in labels:
      if labels[title][0] > 0: # if positive, majority is Yes
        new_tup = (title, labels[title][1])
...
def main():
    ...
    df1.columns = ['title', 'descriptions']
    ...
    # df2.columns = ['Avg. Score', 'Best Description']
    # df2.rename( columns={0 :'Article: Month'}, inplace=True )
